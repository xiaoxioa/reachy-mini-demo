...
**M1c 转向策略(已钉,落地待写码):**
1. DOA 落可用区(±150° 内、读数稳定)→ 直接朝 DOA 转 → 人脸跟随收敛。
2. 落坏区(深后)或**一次声窗内 raw 剧烈跳变**(左右横跳/方差大)→ 判深后不稳 → **转身扫一圈 + 人脸跟随**(白嫖现成视觉跟随,不按 DOA 硬转)。
3. **统一兜底(最终收敛保证):** 任何方向转过去后用"该方向有没有找到人脸"验真;没找到 → 转身扫一圈找人。**视觉是兜底,DOA 错了它来收敛。** 命中转向 + 天线弹确认一并在 M1c 实现。
