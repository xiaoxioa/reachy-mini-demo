...
## 16. M3 (EXP-A) 质量包:运动基础 + 事件反应 + 最小记忆 ✅(2026-06-18)

> 让机器人"感觉活着"的三层叠加:运动拟真(M3-a)、事件反应(M3-b)、最小记忆(M3-c)。每项有 `--no-*` kill switch。

### M3-a 运动基础

| 参数 | 值 | 说明 |
|---|---|---|
| `EASE_ATTACK_FRAC` | 0.35 | cue 攻击占比(easeOutBack 快速上冲带过冲) |
| `BREATH_BLEND_TAU` | 2.0s | 呼吸参数状态间平滑切换时间常数 |
| `CUE_VARIATION` | 0.15 | 每次 cue 触发,dur/pitch/ant 各 ±15% 随机 |

全态呼吸参数(freq Hz, pitch amplitude °):

| 状态 | 频率 | 幅度 | 体感 |
|---|---|---|---|
| ARMED | 0.18 | 2.5 | 慢而明显,休息感 |
| IDLE_CENTER | 0.22 | 1.8 | 中等,待机 |
| TRACKING | 0.25 | 1.0 | 跟脸时轻,不干扰 |
| SEARCHING | 0.25 | 1.0 | 同 TRACKING |
| ENGAGING | 0.20 | 0.5 | 转向中几乎无 |
| RETURNING | 0.22 | 1.5 | 回中,中等 |
| POINTING | 0.20 | 0.5 | 指向中不干扰 |
| PLAYING | 0.30 | 0.8 | 逗它时轻快 |

- **缓动曲线**:cue 包络从 `sin(π·t/dur)` 升级为 easeOutBack(攻击 35%)+ easeInQuad(衰减 65%),过冲让动作更弹性。`--no-easing` 回退原 sin。
- **全态呼吸**:消除 ARMED-only 分支,8 状态各有参数,τ=2s 平滑切换,全局叠加 pitch 偏置。`--no-breathe` 关。
- **微变异**:新 cue 触发时 dur/pitch/ant 各独立随机 ±15%,同一个 "heard" 每次不完全一样。`--no-variation` 关。

### M3-b 事件反应

| 参数 | 值 | 说明 |
|---|---|---|
| `barge_dur` | 0.25s | 打断微反应时长 |
| `barge_pitch` | 2.0° | 打断微后仰幅度 |
| `barge_ant` | 0.3rad | 打断天线收缩幅度 |
| `THINK_ROLL_AMP` | 3.0° | 思考歪头幅度 |
| `THINK_ROLL_F` | 0.15Hz | 思考歪头频率 |
| `THINK_PITCH` | -1.5° | 思考时微抬头 |
| `THINK_ANT_AMP` | 0.15rad | 思考天线不对称摆动 |
| `THINK_ANT_F` | 0.25Hz | 思考天线频率 |
| `THINK_BLEND_TAU` | 0.5s | 思考行为淡入淡出 |
| `EXPR_SMILE_ANT` | +0.20rad | 用户微笑→天线上扬 |
| `EXPR_FROWN_ANT` | -0.15rad | 用户皱眉→天线下垂 |
| `EXPR_BLEND_TAU` | 0.8s | 表情响应平滑常数 |

- **打断微反应**:`_do_barge_in` 触发 "barge" cue(0.25s 微后仰+天线收缩),用户打断时有身体回应而非突然静止。
- **思考歪头**:`speech_stopped` → `thinking=True`,head roll ±3° 慢摆 + 天线不对称摆;收到首段音频自动停止。仅在 TRACKING/IDLE/SEARCHING 态叠加。blend τ=0.5s 平滑进出。
- **表情回应**:vision_worker 启用 `output_face_blendshapes=True`(MediaPipe 52 系数),提取 mouthSmileLeft/Right、mouthFrownLeft/Right 取均值 → 天线偏置(smile +0.20rad 上扬,frown -0.15rad 下垂),τ=0.8s 平滑。cue 活跃时表情不叠加(cue 优先)。
- **天线叠加优先级**:cue > joy(PLAYING) > think + expression > 默认(INIT_ANTENNAS)。
- 全部 `--no-expression` 一键关。

### M3-c 最小记忆

- **被动摘要**:退出时用 `qwen-turbo`(廉价)把 `st.conversation_log` 浓缩为 `data/profile.json`(summary/preferences/topics)。对话太短(<2 轮)跳过。已有画像会合并 preferences。
- **主动记忆**:两个新工具 `remember(content)` / `forget(content)` → `data/memory.v1.json`(id/content/created_at)。forget 按关键词匹配删除。
- **启动注入**:启动时加载 profile + memory,拼入 INSTRUCTIONS 尾部("你的记忆"段)。`--no-memory` 关闭(同时移除工具注册)。
- 数据文件在 `data/` 目录,已 gitignore(隐私)。

### kill switch 汇总

| flag | 关闭项 |
|---|---|
| `--no-easing` | cue 缓动,回退 sin 包络 |
| `--no-breathe` | 全态呼吸 |
| `--no-variation` | cue ±15% 微变异 |
| `--no-expression` | 打断微反应 + 思考歪头 + 表情回应(全部 M3-b) |
| `--no-memory` | 被动摘要 + 主动记忆工具 + 启动注入(全部 M3-c) |
...
| M3 (EXP-A) 缓动+呼吸+变异+打断反应+思考歪头+表情+记忆 | 2026-06-18 | ✅ 验收通过;5 个 kill switch;见 §16 |
