...
INSTRUCTIONS = (
    ...
    "当用户说'向左转''向右转''转过去''转个身''面朝那边'等要求你转向/转身时,"
    "用 turn_body 转动身体。look_left/right 只用于'看看左边有什么'等想看某个方向的画面时。"
    "turn_body 的 angle 参数含义:30°≈小幅转身,45°≈转半边,90°≈转到侧面。"
    "用户说'稍微转一下'给30,'转过去'给45~60,'完全转过去/面朝旁边'给90。"
    "用户说'转回来''面朝我''转过来'时用 turn_body(direction='center') 回到正面并恢复跟随。"
    ...
    "若目标在当前画面外(比如让你看某个方向有什么),可先用 look_left/right/up/down 偏头扫视,"
    "画面会随之更新,再看再答。注意 look_* 只是小幅偏头,不改变身体朝向;要改变朝向用 turn_body。"
...
GAZE_MUTUAL_PITCH_THRESH = 35.0      # mutual 阈值(桌面机器人俯视 → L2CS pitch 系统偏 -30°,13 太紧)
GAZE_DIR_DEADBAND = 8.0              # 方向一致性死区:|head_yaw|<此值时不检查gaze方向(正对相机)
GAZE_L2_EMA_ALPHA = 0.25             # L2 输出 EMA 平滑(越小越平滑,0.25≈4帧有效窗口,压瞬时噪声)
GAZE_MUTUAL_CONFIRM_FRAMES = 3        # mutual_gaze 连续 N 帧L2才确认(对齐后精度高,3帧≈0.5s足够)
...
TURN_BODY_DURATION = 0.8      # turn_body 工具转身动画时长(s)
TURN_BODY_HOLD_S = 2.0        # (已废弃,保留供旧引用) turn_body 后基础 hold 时长
