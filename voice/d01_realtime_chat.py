...
DOA_DEBUG = os.environ.get("DOA_DEBUG") == "1"  # 诊断:打印每次 DOA raw/resid/门(默认关,不改逻辑)
...
class State:
    def __init__(self) -> None:
        ...
        self.wake_fail_until = 0.0     # 连接失败反馈:head_control 在此前渲染天线下垂
...
def doa_sensor_loop(st: State, stop: threading.Event) -> None:
    ...
    while not stop.is_set():
        ...
        if r is not None and r[1] and not robot_speaking:
            buf.append((now, r[0]))
            if DOA_DEBUG:
                log(f"🎧 raw={r[0]:+6.0f}° vad=1 body_yaw={by:+.0f}° (入窗 n={len(buf)})")
        elif DOA_DEBUG and r is not None and r[1] and robot_speaking:
            log(f"🎧 raw={r[0]:+6.0f}° vad=1 (机器人在说话,不入窗)")
        while buf and now - buf[0][0] > SND_WIN_S:
            buf.popleft()
        if len(buf) < SND_MIN_SAMPLES:
            continue
        angles = sorted(a for _, a in buf)
        med = angles[len(angles) // 2]
        resid = 90.0 - med  # 残差:0=正前,+左 -右(相对头当前朝向)
        with st.lock:
            st.sound_resid = resid
            st.sound_at = now
        if DOA_DEBUG:
            gate = abs(resid) >= SND_RESID_MIN
            log(f"🎧→ resid={resid:+.0f}° body_yaw={by:+.0f}° 中值raw={med:+.0f}° n={len(buf)} | "
                f"|resid|≥{SND_RESID_MIN:.0f}? {'是·判视场外→会触发转向' if gate else '否·判正前→不转'}")
...
def behavior_loop(st: State, snap_q: "queue.Queue", stop: threading.Event,
    ...
    while not stop.is_set():
        ...
        if state == ST_ARMED:
            ...
            if woke:
                with st.lock:
                    st.track_yaw = st.track_pitch = 0.0
                set_state(ST_IDLE, seed_interact=True)  # 进 engaged(M1c 改为先 ENGAGING 做 DOA 转向)
            else:
...
def head_control_loop(mini: ReachyMini, st: State, stop: threading.Event) -> None:
    ...
    while not stop.is_set():
        ...
        with st.lock:
            ...
            wake_fail_until = st.wake_fail_until
        if action:
        ...
        # WAKE-01 armed:慢呼吸(小幅低头起伏);连接失败期天线下垂作反馈
        if state == ST_ARMED:
            br = ARMED_BREATH_PITCH * math.sin(2 * math.pi * ARMED_BREATH_F * now)
            ant = ([INIT_ANTENNAS[0] - 0.7, INIT_ANTENNAS[1] - 0.7]
                   if now < wake_fail_until else list(INIT_ANTENNAS))
            try:
                mini.set_target(head=head_pose(pitch_deg=tp + br, yaw_deg=ty),
                    ...
                    if state == ST_ARMED:
                        if conv is not None and not woke_pending:     # engaged→armed:拆 WS(回零连接零计费)
                            ...
                            if conv is not None:
                                ...
                                with st.lock:
                                    st.wake_fail_until = time.monotonic() + 1.5
