...
from voice.conversation import ConversationHandler
from voice.config import (
    VOICE_BACKEND as _VOICE_BACKEND,
    CASCADE_LLM_MODEL, CASCADE_TTS_MODEL, CASCADE_TTS_VOICE,
    CASCADE_ASR_BACKEND, CASCADE_ASR_MODEL,
    SILERO_VAD_MODEL, SHERPA_ASR_MODEL, SHERPA_ASR_TOKENS,
)
...
def snapshot_loop(mini: ReachyMini, st: State, pipeline, oai: OpenAI,
    ...
    while not stop.is_set():
        ...
        try:
            ...
            pipeline.submit_tool_result(
                call_id,
                json.dumps({"success": ok, "scene_description": desc}, ensure_ascii=False),
            )
        except Exception as e:
        ...
        if fire_rc:
            try:
                pipeline.trigger_response()  # 让模型用语音转述所见
            except Exception as e:
                                ...
                                if _actor != _target:
                                    if not (_owner_mgr and _owner_mgr.can_delete_memory(_actor, _target)):
                                        with st.lock:
                                            ...
                                            st.clear_lock = False
                                        log("🔒 权限不足,非主人不能删他人记忆")
                                        if pipeline_ref[0] is not None and pipeline_ref[0].is_connected:
                                            pipeline_ref[0].inject_system_message(
...
def main() -> int:
    ...
    with ReachyMini(
        ...
        try:
            ...
            # ── 管线工厂 ──
            if _VOICE_BACKEND == "cascaded":
                from voice.cascaded.vad_silero import SileroVAD
                from voice.cascaded.llm_openai_compat import OpenAICompatLLM
                from voice.cascaded.tts_cosyvoice import CosyVoiceTTS
                from voice.cascaded.pipeline import CascadedPipeline
                if CASCADE_ASR_BACKEND == "dashscope":
                    from voice.cascaded.asr_dashscope import DashScopeASR
                    _asr = DashScopeASR(model=CASCADE_ASR_MODEL)
                else:
                    from voice.cascaded.asr_sherpa import SherpaASR
                    _asr = SherpaASR(SHERPA_ASR_MODEL, SHERPA_ASR_TOKENS)
                _pipeline = CascadedPipeline(
                    SileroVAD(SILERO_VAD_MODEL), _asr,
                    OpenAICompatLLM(client=oai, model=CASCADE_LLM_MODEL),
                    CosyVoiceTTS(model=CASCADE_TTS_MODEL),
                    voice=CASCADE_TTS_VOICE)
                log(f"🔗 级联管线: VAD=silero ASR={CASCADE_ASR_BACKEND} LLM={CASCADE_LLM_MODEL} TTS={CASCADE_TTS_MODEL}")
            else:
                from voice.qwen_realtime.pipeline import QwenRealtimePipeline
                _pipeline = QwenRealtimePipeline()
                log(f"🔗 Qwen Realtime 管线: {MODEL}")
            ...
            handler = ConversationHandler(
                st, _pipeline, play_q, motion_q, snap_q, _clear_player,
                _memory_mgr, _owner_mgr, _id_recognizer, oai,
                active_instructions, active_tools, no_memory,
                face_pipeline=_face_pipeline)
            _pipeline.set_event_handler(handler.handle_event)

            session_active = False
            kws_gate = None
            if no_wake:
                log("连接语音后端(--no-wake:启动即连)…")
                session_active = handler.open_session(timeout=10.0)
            ...
            if not NO_VOICE:
                threading.Thread(target=snapshot_loop, args=(mini, st, _pipeline, oai, snap_q, stop), daemon=True).start()
            ...
            if _vis_enabled:
                ...
                threading.Thread(target=vision_result_loop, args=(st, vis_result_q, stop, _pipeline_ref), daemon=True).start()
            else:
            ...
            try:
                while True:
                    ...
                    if state == ST_ARMED:
                        if session_active and not woke_pending:
                            handler.close_session()
                            session_active = False
                            log("🌙 已回待命,管线断开(零连接零计费)")
                        elif wake and not session_active:                    # 命中才连(b)
                            ...
                            tc = time.monotonic()
                            session_active = False if sim_fail else handler.open_session()
                            if session_active:
                    ...
                    # engaged:扇出上行给管线
                    if not session_active:
                        # 管线已死但 behavior 还停在 ENGAGING/TRACKING:
                        # KWS 命中时直接重连,否则等 behavior 超时回 ARMED
                        if wake:
                            log("🔔 管线已断但仍在对话态,收到唤醒词 → 重连…")
                            session_active = handler.open_session()
                            if not session_active:
                    ...
                    if _do_greet and not _busy and (time.monotonic() - greet_sent_at) > 1.0:
                        ...
                        if _g_pid and not _g_injected and _memory_mgr is not None:
                            handler.update_memory(_g_pid, _g_pname)
                        _phrase = GREET_PHRASES[greet_i % len(GREET_PHRASES)]
                        greet_i += 1
                        try:
                            _pipeline.trigger_response(instructions=greet_prompt(_phrase))
                    ...
                    # 当前回复仍用旧记忆,导致记忆串人
                    if not no_memory and session_active:
                        with st.lock:
                        ...
                        if _late_pid and not _late_injected and not _late_busy and _memory_mgr is not None:
                            handler.update_memory(_late_pid, _late_pname)
                    ...
                    with st.lock:
                        if st.audio_gate_closed:
                            st.audio_gate_buffer.append(pcm16)
                            # 超时兜底
                    ...
                    if _gate_buf is not None:
                        ...
                        for _gc in _gate_buf:
                            try:
                                _pipeline.feed_audio(_gc)
                            except Exception:
                                break
                    try:
                        _pipeline.feed_audio(pcm16)
                ...
                if not no_memory and _memory_mgr:
                    ...
                    for _exit_pid, _exit_log in _remaining.items():
                        if _exit_pid != "_unknown" and len(_exit_log) >= 2:
                            ...
                            handler.save_summary(_exit_pid, _exit_log)
                log("已释放语音管线与 Reachy 媒体资源。")
                if _memory_mgr is not None:
                    _memory_mgr.flush()
                    log("💾 记忆已持久化")
