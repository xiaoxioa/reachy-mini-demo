...
from perception.gaze import GazeModule
...
from voice.config import (                          # ← 配置常量集中管理
    ...
    HEAD_ASD_EMA, HEAD_SPK_ON, HEAD_SWITCH_MARGIN,
    DOA_GLANCE_DEG, GLANCE_MAX_DEG, GLANCE_MIN_HOLD_S, FPS_FREEZE_BELOW, ASD_MAX_TRACKS,
    GLANCE_TIMEOUT_S, GLANCE_COOLDOWN_S, GLANCE_SPEECH_GRACE_S,
    GLANCE_MIN_TURN_DEG, GLANCE_LOCAL_RMS,
    ...
    BODY_FOLLOW_THRESHOLD, BODY_FOLLOW_SPEED_DPS, TURN_BODY_DURATION,
...
_gaze_mod = None        # GazeModule;main() 初始化(保留引用,用于 ARMED 态开关)
_gaze_fsm = None        # GazeBehaviorFSM;main() 初始化
...
_MEMORY_TOOL_NAMES = ("remember_fact", "clear_memory", "confirm_clear", "forget_fact")
...
def _turn_body_hold_active(st: State) -> bool:
    """turn_body(left/right) 后 body 锁定,直到 center 回正或 idle 超时。"""
    if not st.turn_body_hold:
        return False
    now = time.monotonic()
    if (now - st.last_interaction_at) > NO_INTERACT_S:
        st.turn_body_hold = False
        return False
    return True


def _do_turn_body(mini: ReachyMini, st: State, args: dict) -> None:
    """执行 turn_body 工具:修改 st.body_yaw_deg 并用 goto_target 转身。

    direction=center 时回正(body→0°)并释放 hold,恢复正常跟随。
    direction=left/right 时转向并设 hold flag,持续到 center 或 idle 超时。
    """
    direction = args.get("direction", "left")

    with st.lock:
        old_body = st.body_yaw_deg
        old_yaw = st.track_yaw
        old_pitch = st.track_pitch

    if direction == "center":
        if abs(old_body) < 1.0:
            with st.lock:
                st.turn_body_hold = False
            log("⚠ turn_body(center): 已在中位,释放 hold")
            return
        target = 0.0
        neck_off = old_yaw - old_body
        new_yaw = float(np.clip(target + neck_off, -NECK_REL_LIMIT, NECK_REL_LIMIT))
        duration = max(0.5, TURN_BODY_DURATION * abs(old_body) / 45.0)
        log(f"🔄 turn_body(center): 回正 body {old_body:+.1f}→0°, dur={duration:.2f}s")
        head = head_pose(pitch_deg=old_pitch, yaw_deg=new_yaw)
        mini.goto_target(head=head, body_yaw=math.radians(0.0), duration=duration)
        with st.lock:
            st.body_yaw_deg = 0.0
            st.track_yaw = new_yaw
            st.turn_body_hold = False
        log("✅ turn_body(center) 完成: hold 释放,恢复正常跟随")
        return

    angle = float(args.get("angle", 45.0))
    sign = +1.0 if direction == "left" else -1.0

    target = float(np.clip(old_body + sign * angle, -BODY_LIMIT_DEG, BODY_LIMIT_DEG))
    actual_delta = target - old_body
    if abs(actual_delta) < 1.0:
        log(f"⚠ turn_body: 已在极限,无法再向{direction}转(body={old_body:+.1f}°)")
        return

    neck_off = old_yaw - old_body
    new_yaw = float(np.clip(target + neck_off,
                            target - NECK_REL_LIMIT,
                            target + NECK_REL_LIMIT))
    head = head_pose(pitch_deg=old_pitch, yaw_deg=new_yaw)

    duration = max(0.5, TURN_BODY_DURATION * abs(actual_delta) / 45.0)

    log(f"🔄 turn_body: {direction} {abs(actual_delta):.0f}° "
        f"(body {old_body:+.1f}→{target:+.1f}°, dur={duration:.2f}s)")

    mini.goto_target(head=head, body_yaw=math.radians(target), duration=duration)

    with st.lock:
        st.body_yaw_deg = target
        st.track_yaw = new_yaw
        st.turn_body_hold = True
        st.turn_body_hold_since = time.monotonic()

    log(f"✅ turn_body 完成: body={target:+.1f}° head_yaw={new_yaw:+.1f}° (hold=flag)")
...
def motion_loop(mini: ReachyMini, st: State, motion_q: "queue.Queue", stop: threading.Event) -> None:
    ...
    while not stop.is_set():
        try:
            ...
            continue
        name = job["name"]
        # turn_body 特殊分支:修改 st.body_yaw_deg(普通动作不改状态)
        if name == "turn_body":
            with st.lock:
                st.action_active = True
            try:
                _do_turn_body(mini, st, job.get("args", {}))
            except Exception as e:
                log(f"⚠ turn_body 执行失败:{type(e).__name__}: {e}")
            finally:
                with st.lock:
...
def vision_result_loop(st: State, result_q, stop: threading.Event,
    ...
    _head_ema: dict = {}               # 头部专用二级平滑分(身份键→EMA),只管"头看谁"
    _head_key = None                   # 头部当前跟随的身份(person_id 或 t{track_id}),黏滞抗 churn
    _glance_since = 0.0                 # DOA 偏离持续起点(0=未偏);触发防抖
    _glance_phase = 0                  # 0=off 1=转向声源 2=到位等说话
    _glance_target_yaw = 0.0          # 转头目标世界 yaw(进入时锁定一次=平滑,不 live 追,防来回转)
    _glance_wait_since = 0.0          # 进入"等说话"的时刻
    _glance_cooldown_until = 0.0      # 放弃后冷却,避免对错方向反复转
    _last_speech_t = 0.0             # 最近一次麦 VAD 检测到说话的时刻(转头触发闸,滤掉环境音)
    _glance_log_t = 0.0                # 转头日志节流
    ...
    while not stop.is_set():
        ...
        if msg.get("kind") == "ready":
            ...
            continue
        now = time.monotonic()
        # 瞬时 fps EMA(检测帧到达率):低 fps 时冻结身体跟随/瞟头,阻断"相机甩→churn→fps更低"死循环
        if _last_frame_t > 0.0:
            _dtf = now - _last_frame_t
            if _dtf > 1e-3:
                _fps_ema = 0.3 * (1.0 / _dtf) + 0.7 * _fps_ema
        _last_frame_t = now
        _cam_ok = _fps_ema >= FPS_FREEZE_BELOW   # 相机运动(身体跟随/瞟头)许可:fps 够才动
        if not _cam_ok and now - _lowfps_log_t >= 2.0:
            _lowfps_log_t = now
            log(f"🧊 低 fps({_fps_ema:.1f})→ 冻结身体跟随/瞟头,等跟踪恢复(断 churn 死循环)")
        ...
        if _face_pipeline is not None and all_faces is not None:
            ...
            if _raw_frame is not None:
                try:
                    ...
                    # ── gaze 仅 ARMED 态运行(L2CS ~35ms/face,对话态不需要) ──
                    _face_pipeline._gaze = _gaze_mod if st.state == ST_ARMED else None
                    _primary, _track_views = _face_pipeline.process(
                        all_faces, (_dw, _dh), _full_rgb, DECIMATE, now, _doa_selected_idx)
                    # ── ASD:按「身份键」聚合喂帧(churn 换 id 也喂同一缓冲,治新人画外)。
                    #    ①只喂最大的 ASD_MAX_TRACKS 个 track(churn 出一堆 ghost 时逐个裁剪+打分会爆 CPU/GPU→fps崩);
                    #    ②fps 低(_cam_ok False)直接跳过 ASD,把资源让给检测先恢复(治长时间低 fps)。
                    if _asd_engine is not None and _asd_engine.available and _track_views and _cam_ok:
                        _feed = sorted(_track_views,
                                       key=lambda v: (v.bbox_px[2] - v.bbox_px[0]) * (v.bbox_px[3] - v.bbox_px[1]),
                                       reverse=True)[:ASD_MAX_TRACKS]
                        for _v in _feed:
                            _b = _v.bbox_px
                            _bbox_full = (_b[0] * DECIMATE, _b[1] * DECIMATE,
                                          _b[2] * DECIMATE, _b[3] * DECIMATE)
                            _k = _v.person_id or f"t{_v.track_id}"
                            _asd_engine.feed_crop(_k, _full_rgb, _bbox_full, now, track_id=_v.track_id)
                        _asd_engine.gc([(_v.person_id or f"t{_v.track_id}") for _v in _feed])
                    ...
                    _scs_all = _asd_engine.scores() if _asd_on else {}
                    if _asd_on and now - _asd_log_t >= 3.0:    # 周期打印 ASD 分(便于无 dashboard 验证)
                        _asd_log_t = now
                        if _scs_all:
                            _spk_k = _spk[0] if _spk else None
                            log("🗣 ASD: " + "  ".join(
                                f"{'▶' if k == _spk_k else ' '}T{_asd_engine.last_track(k) or '?'}={s:+.2f}"
                                for k, s in sorted(_scs_all.items(), key=lambda kv: str(kv[0]))))
                    # 维护"画面内 ASD 说话人"(供 user query 归属;None=画外/无人说话)。_spk[0]=身份键
                    _asp_set = None
                    if _spk is not None and _track_views:
                        _sv = next((v for v in _track_views
                                    if (v.person_id or f"t{v.track_id}") == _spk[0]), None)
                        if _sv is not None:
                    ...
                    elif not _asd_on and _primary is not None:
                        # ASD 不可用时回退到最大脸(单人场景等价;多人场景宁可归属最大脸也不画外)
                        with st.lock:
                            _pname_fb = _primary.person_name
                            if _memory_mgr is not None and _primary.person_id:
                                _pname_fb = _memory_mgr.get_name(_primary.person_id) or _pname_fb
                            st.asd_speaker = {
                                "pid": _primary.person_id,
                                "name": _pname_fb,
                                "track_id": _primary.track_id,
                                "score": 0.0,       # 无 ASD 分数
                                "at": now,
                            }
                    # ── 头部"看谁" + 当前人:都按"身份(person_id)"黏滞,抗 track churn ──
                    #    治:①无人说话时头回退最大脸帧间翻=晃;②current_person 随瞬时 ASD 抖动反复
                    #    update_session 重注入→竞态→"归属对但回复叫错人"。归属/记忆保存仍走 speaker_window(敏感)。
                    _key2view = {}                            # 身份键 → 该身份当前 track 视图(首个即可,同身份分数相同)
                    for _v in (_track_views or []):
                        _k = _v.person_id or f"t{_v.track_id}"
                        if _k not in _key2view:
                            _key2view[_k] = _v
                    for _k, _v in _key2view.items():          # 二级重 EMA(更平滑 ASD),按身份累积
                        _inst = _scs_all.get(_k, -1.0)        # 分数已按身份键聚合
                        _pe = _head_ema.get(_k)
                        _head_ema[_k] = _inst if _pe is None else (HEAD_ASD_EMA * _inst + (1.0 - HEAD_ASD_EMA) * _pe)
                    for _gk in [k for k in _head_ema if k not in _key2view]:
                        _head_ema.pop(_gk, None)              # 清掉离场身份
                    if _head_key is not None and _head_key not in _key2view:
                        _head_key = None                      # 跟随对象离场才释放(churn 换 track 不算,身份还在)
                    _cand = max(_head_ema.items(), key=lambda kv: kv[1]) if _head_ema else None
                    if _cand is not None and _cand[1] > HEAD_SPK_ON:       # 有人明确说话才考虑切
                        if _head_key is None or (_cand[0] != _head_key and
                                _cand[1] > _head_ema.get(_head_key, -9.9) + HEAD_SWITCH_MARGIN):
                            _head_key = _cand[0]              # 切到明确说话人(需高出当前余量,防来回甩)
                    if _head_key is None and _primary is not None:
                        _head_key = _primary.person_id or f"t{_primary.track_id}"   # 无人说话且无跟随→锁最大脸(黏滞)
                    _head_view = _key2view.get(_head_key) if _head_key is not None else None
                    if _head_view is None:
                        _head_view = _primary
                    if _head_view is not None:
                        u_raw, v_raw = _head_view.u, _head_view.v
                    # ── 注视行为 FSM(仅 ARMED 态运行;对话态不需要 gaze) ──
                    if _gaze_fsm is not None and _track_views and st.state == ST_ARMED:
                    ...
                    elif _gaze_fsm is not None and st.state != ST_ARMED:
                        # 非 ARMED 态:清空 gaze 状态,防 behavior_loop 读到旧值阻塞状态转换
                        with st.lock:
                            st.gaze_behavior = "IDLE"
                    ...
                    if _head_view is not None and _head_view.person_id is not None:
                        _pid_now = _head_view.person_id
                        _pname = _head_view.person_name
                        if _memory_mgr is not None:
                            _mn = _memory_mgr.get_name(_pid_now)
                            if _mn:
                                _pname = _mn
                        with st.lock:
                            old_pid = st.current_person_id
                            old_name = st.current_person_name
                            st.current_person_id = _pid_now
                            st.current_person_name = _pname
                            st.current_is_owner = (_owner_mgr.is_owner(_pid_now) if _owner_mgr else False)
                        if _pid_now != old_pid or _pname != old_name:   # 切人或改名才记日志,避免刷屏
                            log(f"🆔 {_pname or _pid_now[:12]} (track {_head_view.track_id}/{_head_view.zone}, 焦点)")
                except Exception as e:
        ...
        if u_raw is not None:
            ...
            if integrate:
                # ── DOA 转头找人(状态机):转到声源角度 → 停那等说话 → 锁说话人;找不到不弹回原大脸。
                #    平滑:目标进入时锁定一次(不 live 追)+ 放弃后冷却,避免来回转。
                _spk_now = _asd_engine.speaker() if (_asd_engine is not None and _asd_engine.available) else None
                with st.lock:
                    _gr = st.doa_resid_stable
                    _doa_ok = (_gr is not None and st.doa_confident
                               and (now - st.doa_at) < DOA_GATE_FRESH_S)
                    if st.user_speaking:
                        _last_speech_t = now
                    _local_speech_at = st.local_speech_at   # F1:门控前本地响度信号
                # 触发:DOA偏+确信 + 画面内没人在说话 + fps够 + 冷却过 + 最近真有人说话(滤环境音,治无声左漂)
                # F1:realtime VAD(被门控掐)或 本地麦响度(未门控,绕开宽角度死锁)任一新鲜即算"有人说话"
                _recent_speech = ((now - _last_speech_t) < GLANCE_SPEECH_GRACE_S
                                  or (now - _local_speech_at) < GLANCE_SPEECH_GRACE_S)
                _want = (_doa_ok and abs(_gr) > DOA_GLANCE_DEG and _spk_now is None
                         and _cam_ok and _recent_speech and now >= _glance_cooldown_until
                         and not _turn_body_hold_active(st))
                if _glance_phase == 0:
                    if _want:
                        if _glance_since == 0.0:
                            _glance_since = now
                        if (now - _glance_since) >= GLANCE_MIN_HOLD_S:
                            _glance_phase = 1             # → 转向(目标锁定一次=平滑)
                            # F4:DOA 角度不可信(项目结论:只信符号不信大小)→ 朝符号方向转「足够大的固定角」
                            #     (至少 GLANCE_MIN_TURN_DEG,封顶 GLANCE_MAX_DEG),把宽角度的人转进视野;治"转向不够"。
                            _turn_mag = float(np.clip(max(abs(_gr), GLANCE_MIN_TURN_DEG), 0.0, GLANCE_MAX_DEG))
                            with st.lock:
                                _glance_target_yaw = st.body_yaw_deg + math.copysign(_turn_mag, _gr)
                            _head_key = None              # 忘掉原焦点:等待/放弃都不弹回原来的大人脸
                            _glance_log_t = now
                            log(f"👀 DOA 转头找人(resid {_gr:+.0f}° → 转 {math.copysign(_turn_mag, _gr):+.0f}° 到 yaw {_glance_target_yaw:+.0f}°,停那等说话)")
                    else:
                        _glance_since = 0.0
                else:  # 转向 / 等待中
                    if (_head_key is not None and _head_key in _key2view
                            and _head_ema.get(_head_key, -9.9) > HEAD_SPK_ON):
                        _glance_phase = 0                 # 画面内有人明确说话 → 锁定,正常跟随接管
                        log("👀 锁定说话人 → 跟随接管")
                    else:
                        with st.lock:
                            _arrived = abs(st.track_yaw - _glance_target_yaw) < 8.0
                        if _glance_phase == 1 and _arrived:
                            _glance_phase = 2             # → 到位,停那等说话
                            _glance_wait_since = now
                        if _glance_phase == 2 and (now - _glance_wait_since) > GLANCE_TIMEOUT_S:
                            _glance_phase = 0             # 等超时没人说话 → 放弃(保持当前朝向,不弹回原脸)
                            _glance_cooldown_until = now + GLANCE_COOLDOWN_S
                            log("👀 转头后无人说话 → 放弃,保持当前朝向")
                if _glance_phase in (1, 2):
                    with st.lock:
                        err_yaw = _glance_target_yaw - st.track_yaw   # 头朝锁定的 DOA 目标(超颈限→身体跟随转过去);不弹回 primary
                    err_pitch = 0.0
                else:
                ...
                with st.lock:
                    ...
                    # turn_body hold:用户主动转身后 hold 期间 body 不自动追回
                    # 低 fps 时冻结身体(身体甩动=最大相机自运动→运动模糊→track churn→fps 更低,死循环)
                    neck_off = st.track_yaw - st.body_yaw_deg
                    threshold = NECK_REL_LIMIT * BODY_FOLLOW_THRESHOLD
                    _glance_active = _glance_phase in (1, 2)
                    _hold_active = _turn_body_hold_active(st)
                    if abs(neck_off) > threshold and (_cam_ok or _glance_active) and not _hold_active:
            ...
            miss_streak += 1
            t_prev_ctrl = now
            _glance_since = 0.0; _glance_phase = 0            # 无脸:清转头状态
            hit_window.append(0)
            _rate = sum(hit_window) / len(hit_window) if hit_window else 0.0
        ...
        if VIS_DEBUG:
            ...
            _tv = []
            if _track_views:
                _asd_on2 = _asd_engine is not None and _asd_engine.available
                _scs2 = _asd_engine.scores() if _asd_on2 else {}
                _spk_ids = _asd_engine.speaking_ids() if _asd_on2 else set()
                for v in _track_views:
                    _vk = v.person_id or f"t{v.track_id}"       # 身份键(与 ASD 聚合一致)
                    _asc = _scs2.get(_vk)
                    # 显示名每帧从实时记忆库现取(改名立即跟上,治缓存滞后);取不到再退回 pipeline 缓存名
                    _live_name = ((_memory_mgr.get_name(v.person_id) if (v.person_id and _memory_mgr is not None) else None)
                                  or v.person_name)
                    _tv.append({
                        "box": v.bbox_px,           # [x1,y1,x2,y2] 降采样像素
                        "track_id": v.track_id,
                        "name": _live_name,         # 实时库名 / Unknown-N / None(未绑定)
                        "zone": v.zone,
                        "confirmed": v.is_confirmed,
                        "selected": (_vk == _head_key),          # = 头部跟随对象(供注册面板高亮)
                        "asd": round(_asc, 2) if _asc is not None else None,
                        "speaking": (_vk in _spk_ids),           # 确信在说话:>阈值且新鲜(治残留绿)
                        "mutual_gaze": v.mutual_gaze,
                        "head_yaw": round(v.head_yaw, 1),
                        "gaze_yaw": round(v.gaze_yaw, 1),
                        "gaze_pitch": round(v.gaze_pitch, 1),
                    })
            with st.lock:
                st.dbg_det = {
                    "face": msg.get("face"),
...
def behavior_loop(st: State, snap_q: "queue.Queue", stop: threading.Event,
    ...
    def approach(ty_goal: float, body_goal: float, tp_goal: float) -> bool:
        ...
        turn_body hold 期间 body_yaw_deg 冻结(不被 approach 拉回)。"""
        with st.lock:
            ty, b, tp = st.track_yaw, st.body_yaw_deg, st.track_pitch
            hold = _turn_body_hold_active(st)
            def mv(cur, goal):
                d = goal - cur
            ...
            st.track_pitch = mv(tp, tp_goal)
                    ...
                    (hold or abs(st.body_yaw_deg - body_goal) < 0.5) and
    ...
    while not stop.is_set():
        ...
        if state == ST_IDLE:
            ...
            # turn_body hold 期间不因丢脸跳 SEARCHING(转身后短暂丢脸是正常的)
            _tb_hold = _turn_body_hold_active(st)
            if not locked and not _tb_hold:
            ...
            elif snd is not None and not speaking and not _turn_body_hold_active(st):
                # turn_body hold 期间不响应声源(转身后丢脸是预期的,不应该循声源转回去)
                with st.lock:
                    engage_target = float(np.clip(st.track_yaw + snd, -SND_TARGET_LIMIT, SND_TARGET_LIMIT))
                wide_scan = False
                set_state(ST_ENGAGING)
...
def main() -> int:
    ...
    _face_pipeline._gaze = _gaze_mod
    log("👁 Gaze 就绪(L2CS-Net MobileNetV2)" if _gaze_mod.available
        else "⚠ Gaze L2 不可用(缺 ONNX 权重)→ 仅用 L0 头姿")
    global _gaze_fsm
    _gaze_fsm = GazeBehaviorFSM(
        idle_timeout_s=GAZE_IDLE_TIMEOUT_S,
                ...
                while True:
                    ...
                    if conv is None:
                        ...
                        if wake:
                            ...
                            time.sleep(0.01)
                        continue
                    # M1.5-b 二次唤醒(A方案:喊"小艺"→ 打断当前回话 + 天线应答 + 转向 DOA 找喊话人,保留会话):
                    if wake and not no_switch and (time.monotonic() - last_switch) > SWITCH_COOLDOWN_S:
                        if st.clear_lock:
                            ...
                            # ① 立刻打断当前回话(停播放 + 取消在途回复)
                            with st.lock:
                                _in_flight = st.in_flight > 0
                            try:
                                dialog.callback._do_barge_in(_in_flight)
                            except Exception as _be:
                                log(f"⚠ 唤醒打断失败:{type(_be).__name__}: {_be}")
                            # ② 天线上扬应答("听到了,我来找你")——覆盖 barge cue
                            with st.lock:
                                st.wake_cue = "heard"
                                st.wake_cue_t = time.monotonic()
                            ...
                            # ③ 转向 DOA 方向找喊话人(behavior 三档:confident直转/粗方向/反向),保留会话
                            with st.lock:
                    ...
                    # F1:本地麦克风响度(门控前的原始音)→ DOA 瞟头的"有人说话"信号。
                    # 绕开方向门控对 >55° 声音的静音(否则 realtime VAD 收不到 → 瞟头永不触发,治"侧边喊不转")。
                    # in_flight==0 防把机器人自己的回话/回声当成用户。
                    if _rms > GLANCE_LOCAL_RMS:
                        with st.lock:
                            if st.in_flight == 0:
                                st.local_speech_at = now_g
